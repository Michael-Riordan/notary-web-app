...
export default function Admin() {
    ...
    const validateCredentials = async (event) => {
        ...
        try {
             const response = await axios.post(`${import.meta.env.SERVER_DOMAIN}/credentials`, {
                username,
                password,
            });
    ...
    const updateTime  = (path) => {
        ...
        if (validateTimeFormat(hourToUpdate)) {
            ...
            if (path === 'delete-hours'? dayToUpdate.includes(hourToChange) : !dayToUpdate.includes(hourToChange)) {
                fetch(`${import.meta.env.SERVER_DOMAIN}/${path}`, {
                    method: 'POST',
                    headers: {
    ...
    const updateAppointmentStatus = () => {
        ...
        axios.put(`${import.meta.env.SERVER_DOMAIN}/updateAppointment/${selectedAppointmentId}`, { status: newStatus})
             .then((response) => {
    ...
    const removeFromPending = () => {
        ...
        fetch(`${import.meta.env.SERVER_DOMAIN}/removePendingAppointment`, {
            method: 'POST', 
            headers: {
                'Content-Type': 'application/json', 
            },
    ...
    const updatePendingAppointments = async (event) => {
        ...
        if (event.target.textContent === 'Reject Appointment') {
            removeFromPending();
            await axios.delete(`${import.meta.env.SERVER_DOMAIN}/deleteAppointment/${selectedAppointmentId}`)
        } else {
    ...
    useEffect(() => {
        ...
        const fetchTimes = async () => {
            const results = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/business-hours`);
    ...
    useEffect(() => {
        const fetchBlockedDates = async () => {
            const results = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/blocked-dates`);
    ...
    useEffect(() => {
        const fetchBlockedDateAndTime = async () => {
            const results = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/blocked-time-for-date`)
    ...
    useEffect(() => {
        const fetchPendingAppointments = async () => {
            const results = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/pending-appointments`)
    ...
    useEffect(() => {
        const getAppointments = async () => {
            await axios.get(`${import.meta.env.SERVER_DOMAIN}/appointments`)
        ...
        appointments.forEach(appointment => {
            if (pendingAppointments.length === 0 && pendingAppointmentAccepted === false && appointment.status !== 'Accepted') {
                ...
                axios.delete(`${import.meta.env.SERVER_DOMAIN}/deleteAppointment/${appointment.appointmentid}`)  
            } else {
