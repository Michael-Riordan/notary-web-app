...
export default function Quote() {
    ...
    const changeAppointmentRequest = (event) => {
        ...
        if (appointmentRequested == null) {
            ...
            if (value === 'yes' && appointment === '') {
                ...
                saveToSessionStorage();
                axios.delete(`${import.meta.env.SERVER_DOMAIN}/deleteAppointment/${appointmentId}`)
                history.push('./appointment')
            } else {
                return
            };
    ...
    const sendEmail = (event) => {
        ...
        fetch(`${import.meta.env.SERVER_DOMAIN}/updatePendingAppointments`, {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        const fetchDistance = async () => {
            if (place_id != '') {
                const response = await fetch(`${import.meta.env.SERVER_DOMAIN}/api/distance?query=${encodeURIComponent(place_id)}`);
