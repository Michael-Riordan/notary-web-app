...
export default function Quote() {
    ...
    const sendEmail = (event) => {
        ...
        fetch(`${import.meta.env.VITE_SERVER_DOMAIN}/updatePendingAppointments`, {
            method: 'POST',
            headers: {
    ...
    useEffect(() => {
        const fetchDistance = async () => {
            if (place_id != '') {
                const response = await fetch(`${import.meta.env.VITE_SERVER_DOMAIN}/api/distance?query=${encodeURIComponent(place_id)}`);
            ...
            <section id='quote-body'>
                ...
                <div id='quote-calculator'>
                    ...
                    <form className='input-wrapper' onSubmit={sendEmail}>
                        ...
                        <div>
                            {emailSent == null? '' : !emailSent? <p style={{color: 'red'}}>Email Failed to Send, Please Try Again Later</p> : ''}
                        </div>
                    </form>
                </div>
            </section>
